...
def build_layer(mps_data, target_dim=4, layer_idx=0):
    ...
    layer_qc.append(UnitaryGate(U_list[0].conj().T, label="U1_layer"+str(layer_idx)), first_gate_qubits)
    for i in range(1, n_sites - 1):
        start_index = 2 + 4 * (i - 1)
    ...
    layer_qc.append(UnitaryGate(U_list[-1].conj().T, label="U"+str(n_sites)+"_layer"+str(layer_idx)), last_gate_qubits)
