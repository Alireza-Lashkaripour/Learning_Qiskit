...
def build_circuit_from_mps(mps_data, target_dim=2):
    ...
    qc.append(UnitaryGate(unitaries[0], label="U1_dg"), [0, 1])
    for i in range(1, n_sites - 1):
        qc.append(UnitaryGate(unitaries[i], label="U{}_dg".format(i + 1)), [i, i + 1])
    qc.append(UnitaryGate(unitaries[-1], label="U{}_dg".format(n_sites)), [num_qubits - 2, num_qubits - 1])
