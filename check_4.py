...
# Create a quantum MPS with 7 qudits (qubits)
quantum_mps = mpsim.MPS(nqudits=n_sites)

for i in range(n_sites):
    quantum_mps.h(i)
    pdm_diag = mps_data['pdm1'][i,i]
    angle = complex.pi * pdm_diag/2  # Scale based on site occupation
    quantum_mps.ry(i, angle)

for i in range(n_sites-1):
    quantum_mps.cnot(i, i+1, maxsvals=bond_dims[i+1])

wavefunction = quantum_mps.wavefunction
print(f"Created quantum MPS with {n_sites} sites")
print(f"Wavefunction dimension: {len(wavefunction)}")
print(f"First few amplitudes: {wavefunction[:4]}")
