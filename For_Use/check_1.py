...
def complete_unitary_from_fixed_row(v):
    # Completes a normalized vector v (in C^n) into an n×n unitary.
    n = v.shape[0]
...
def complete_unitary_from_fixed_row_2x2(v):
    # Given a 2-dimensional normalized vector v, construct a 2×2 unitary
    # whose first row (up to complex conjugation) equals v.
    # A standard choice for the orthogonal complement is: u = [-v[1], v[0]]
...
def complete_gate_for_site(tensor, site_type):
    ...
    if site_type in ["first", "intermediate"]:
        # For sites where the gate should be a two-qubit unitary (4x4)
        if tensor.ndim < 2:
...
def truncate_to_target(tensor, site_type, target=2):
    # Truncate the tensor to the desired target dimensions.
...
def pick_largest_block(tensor, qlabels):
    # If tensor is a plain numpy array, return it;
    # otherwise, if it is structured, select the block with largest norm.
...
def compute_unitaries_from_mps(mps_data, target_dim=2):
    # Build the list of unitary gates (one per site) from the MPS data.
...
def build_deep_circuit_from_mps(mps_data, target_dim=2, num_layers=1):
    # Construct a quantum circuit that applies 'num_layers' layers.
    # Each layer applies the sequence of unitary gates derived from the MPS.
    n_sites = mps_data['n_sites']
...
# -------------------------
# Simulation of the deep circuit
# -------------------------

# Assume mps_data has been saved by pyblock3 in "h2o_mps_complete.npy"
# and contains keys: 'n_sites', 'tensors', 'q_labels', and 'energy'.
# For example:
mps_data = np.load("h2o_mps_complete.npy", allow_pickle=True).item()

# Build a deep quantum circuit with the desired number of layers.
num_layers = 2  # Change this value as needed for multiple layers.
qc = build_deep_circuit_from_mps(mps_data, target_dim=2, num_layers=num_layers)
print(qc.draw(output="text"))

# Use the density matrix simulator from Qiskit Aer.
simulator = AerSimulator(method="density_matrix")
compiled_circuit = transpile(qc, simulator)
job = simulator.run(compiled_circuit, shots=1)
result = job.result()

# Retrieve the density matrix saved during the run.
rho = result.data(0)["rho"]
density = DensityMatrix(rho)

# For demonstration, we construct a dummy Hamiltonian: here we use the DMRG energy
# stored in mps_data and an operator equal to (energy_target × Identity).
# (In a realistic setting one would map the physical Hamiltonian to qubits.)
energy_target = mps_data['energy']
n_qubits = qc.num_qubits
H_dummy = energy_target * SparsePauliOp.from_list([("I" * n_qubits, 1.0)])
H_matrix = H_dummy.to_matrix()
exp_value = np.real(np.trace(H_matrix @ density.data))
print("Measured energy from density matrix simulation: {:.12f}".format(exp_value))
