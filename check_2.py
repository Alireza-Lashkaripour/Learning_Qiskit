...
for i in range(n_sites):
    occupation = pdm1[i, i] / 2  # Normalize to range 0-1 (since max occupation is 2)
    angle = np.pi * occupation   # Map to rotation angle
    qc.ry(angle, i)
    if i < n_sites - 1:
        qc.cx(i, i+1)

print(f"Created quantum circuit with {n_sites} qubits")
...
print(f"Obtained quantum state with dimension: {quantum_state.dim}")
print(f"First few amplitudes: {quantum_state.data[:7]}")
